...
| **OpenAI API** (GPT-4o-mini) | Proveedor cloud de LLM | Latencia más baja de los tres proveedores (5.1 s); JSON limpio sin envoltorios de Markdown; separación nativa de roles en el array `messages[]`; desempeño global de 8.06/10 | Dependencia de conectividad externa; costos por token; dos fallas en casos con prioridad de frío (EVAL-003 y EVAL-005) | La respuesta en JSON limpio eliminó la etapa de extracción por regex en el `ResponseParser`, simplificando el parseo respecto a Google; su latencia lo posiciona como alternativa viable para flujos interactivos de producción |
...
La latencia de 188 s registrada por Ollama, que lo descarta como opción para flujos interactivos en tiempo real, no representa un impedimento en el contexto de la logística agrícola de distribución. La planeación del transporte de cosechas opera sobre horizontes medidos en días: la ventana de cosecha, el corredor vial y el tipo de producto se conocen con antelación suficiente para que la solicitud de recomendación se resuelva fuera del camino crítico de la operación. En ese escenario, el costo cero por inferencia de Ollama adquiere relevancia estratégica, en particular en un periodo en el que los proveedores comerciales de modelos de lenguaje han ajustado sus planes de facturación hacia esquemas por token que generan costos variables difíciles de presupuestar para una operación agrícola de escala mediana. El puntaje global de 7.19/10, aunque inferior al de los proveedores comerciales, evidenció que un SLM local con un prompt cuidadosamente diseñado y validado en modo estricto produce recomendaciones estructuralmente correctas en la totalidad de las solicitudes. La trazabilidad que ofrece Langfuse sobre cada inferencia complementa esta viabilidad: los errores se hacen visibles, se pueden diagnosticar con evidencia concreta y el prompt puede corregirse de forma iterativa sin incurrir en costos adicionales por las pruebas. Esta dinámica de mejora continua, sostenida por la observabilidad, convierte a Ollama en una opción técnicamente justificable para organizaciones que priorizan la soberanía de datos y el control de costos sobre la latencia de respuesta.
