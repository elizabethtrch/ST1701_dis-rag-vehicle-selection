...
La corrección del `GoogleAdapter` para emplear `system_instruction` en lugar de concatenación textual evidenció un principio aplicable a todos los adaptadores del sistema: la separación de roles de prompt no debe delegarse a la capacidad interpretativa del modelo. Los LLMs comerciales de gran capacidad toleran la ausencia de esta separación formal sin pérdida de calidad medible en los resultados de evaluación. Los SLMs con ventanas de contexto reducidas y menor capacidad de seguimiento de instrucciones implícitas son más sensibles a esta distinción: un adaptador que entrega sistema y usuario como texto homogéneo transmite una señal ambigua que incrementa la probabilidad de que el modelo ignore las restricciones del dominio en favor del contenido del pedido. Honrar la separación de roles a nivel de API, aprovechando los mecanismos nativos de cada SDK, es una práctica que protege la adherencia a las instrucciones con independencia del proveedor o del tamaño del modelo desplegado.
